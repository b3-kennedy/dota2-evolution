...
function spawn_assassin:OnSpellStart()
    ...
    local testunit = CreateUnitByName("npc_dota_custom_phantom_assassin", self.enemy_spawn, true, self:GetCaster(), nil, DOTA_TEAM_BADGUYS)
...
function spawn_assassin:GetAbilityTag()
    return "spawner"
end

function spawn_assassin:SetUpUnitData()
    self.create_unit_modifier = self:GetCaster():FindModifierByName("create_unit_modifier")
    self.spawn_pos = self:GetCaster():FindModifierByName("create_unit_modifier").spawn_pos
    self.enemy_spawn = self:GetCaster():FindModifierByName("create_unit_modifier").enemy_spawn
...
function spawn_assassin:SpawnLevelOne()
    self:SetUpUnitData()
    self.unit_data.ability1 = "far_blink"
    if self.team == DOTA_TEAM_GOODGUYS then
        self.unit_data.name = "npc_dota_custom_phantom_assassin"
    else
        self.unit_data.name = "npc_dota_custom_phantom_assassin"
    end
