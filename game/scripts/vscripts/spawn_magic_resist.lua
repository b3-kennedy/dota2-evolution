spawn_magic_resist = class({})

require('libraries/timers')

LinkLuaModifier("spawner_modifier", LUA_MODIFIER_MOTION_NONE)

function spawn_magic_resist:OnSpellStart()
    ...
    local testunit = CreateUnitByName("npc_dota_custom_slardar", self.enemy_spawn, true, self:GetCaster(), nil, DOTA_TEAM_BADGUYS)
...
function spawn_magic_resist:GetAbilityTag()
    return "spawner"
end

function spawn_magic_resist:SetUpUnitData()
    self.create_unit_modifier = self:GetCaster():FindModifierByName("create_unit_modifier")
    self.spawn_pos = self:GetCaster():FindModifierByName("create_unit_modifier").spawn_pos
    self.enemy_spawn = self:GetCaster():FindModifierByName("create_unit_modifier").enemy_spawn
...
function spawn_magic_resist:SpawnLevelOne()
    self:SetUpUnitData()
    self.unit_data.ability1 = "magic_resist"
    if self.team == DOTA_TEAM_GOODGUYS then
        self.unit_data.name = "npc_dota_custom_slardar"
    else
        self.unit_data.name = "npc_dota_custom_slardar"
    end
