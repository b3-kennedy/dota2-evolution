treant_protector_modifier = class({})

require('positions')

function treant_protector_modifier:OnCreated(kv)

    if not IsServer() then return end
    self:StartIntervalThink(1.5)
    self.parent = self:GetParent()
    self.ability = self:GetParent():GetAbilityByIndex(0)
    self.target = nil
...
function treant_protector_modifier:OnIntervalThink()


    -- First check if ability is ready before searching for targets
    if not self.ability or not self.ability:IsCooldownReady() then
