...
function spawn_lina:OnSpellStart()
    ...
    local testunit = CreateUnitByName("npc_dota_custom_lina", self.enemy_spawn, true, self:GetCaster(), nil, DOTA_TEAM_BADGUYS)
...
function spawn_lina:GetAbilityTag()
    return "spawner"
end

function spawn_lina:SetUpUnitData()
    self.create_unit_modifier = self:GetCaster():FindModifierByName("create_unit_modifier")
    self.spawn_pos = self:GetCaster():FindModifierByName("create_unit_modifier").spawn_pos
    self.enemy_spawn = self:GetCaster():FindModifierByName("create_unit_modifier").enemy_spawn
...
function spawn_lina:SpawnLevelOne()
    self:SetUpUnitData()
    self.unit_data.ability1 = "lina_dragon_slave"
    if self.team == DOTA_TEAM_GOODGUYS then
        self.unit_data.name = "npc_dota_custom_lina"
    else
        self.unit_data.name = "npc_dota_custom_lina"
    end
