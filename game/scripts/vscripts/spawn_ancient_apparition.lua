spawn_ancient_apparition = class({})
...
function spawn_ancient_apparition:OnSpellStart()
    ...
    local testunit = CreateUnitByName("npc_dota_custom_ancient_apparition", self.enemy_spawn, true, self:GetCaster(), nil, DOTA_TEAM_BADGUYS)
...
function spawn_ancient_apparition:GetAbilityTag()
    return "spawner"
end

function spawn_ancient_apparition:SetUpUnitData()
    self.create_unit_modifier = self:GetCaster():FindModifierByName("create_unit_modifier")
    self.spawn_pos = self:GetCaster():FindModifierByName("create_unit_modifier").spawn_pos
    self.enemy_spawn = self:GetCaster():FindModifierByName("create_unit_modifier").enemy_spawn
...
function spawn_ancient_apparition:SpawnLevelOne()
    self:SetUpUnitData()
    --self.unit_data.ability1 = "far_blink"
    if self.team == DOTA_TEAM_GOODGUYS then
        self.unit_data.name = "npc_dota_custom_ancient_apparition"
    else
        self.unit_data.name = "npc_dota_custom_ancient_apparition"
    end
